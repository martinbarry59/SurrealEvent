...
class dataviewer:
    ...
    def extractEvents(self, reversex = False):
        ...
        ys = self.events["y"]
        ps = self.events["polarity"]* 2 - 1 if reversex else  self.events["p"]* 2 - 1
    ...
    def selectEvents(self, filtered_events):
        ...
        if self.events is None:
            ...
            self.events = np.concatenate((self.events, filtered_events), axis=0).copy()
        self.events = self.events[-self.max_events:].copy() # Keep only the last N events
...
class dataviewer39(dataviewer):
    def __init__(self):
        print("Using dv_processing for event processing")
        super().__init__()
        
        self.reader = dv.io.CameraCapture()
...
class dataviewer312(dataviewer):
    def __init__(self):
        super().__init__()
